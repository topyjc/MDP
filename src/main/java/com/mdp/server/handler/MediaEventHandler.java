    ...
    private void process(String topic, byte[] payload) {
        ...
        try {
            ...
            if (confidence >= 0.9) {
                ...
                sendAlertToAppAndWeb(teamId, analysisType, fullImageUrl, "실시간 위험 감지");
                ...
                if (isRoadLicensePlate) {
                    ...
                    if (aiResult != null && aiResult.contains("detected=true")) {
                        ...
                        sendAlertToAppAndWeb(teamId, analysisType, fullImageUrl, "AI 분석 결과, 위험 상황 확정");
                        isDangerDetected = true;
                    } else {
...
//                    controlService.sendTrafficLightCommand(); // 이거는 이제 Sensor에서 호출해야함(위치 데이터를 전송하기 위해)
    ...
    private void sendAlertToAppAndWeb(String teamId, String type, String url, String message) {
