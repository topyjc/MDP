...
import com.mdp.server.service.JWTService; // 🔥 추가됨
...
public class UserController {
    ...
    private final JWTService jwtService; // 🔥 토큰 발급기 추가

    // 생성자로 주입받기
    public UserController(DataService dataService, JWTService jwtService) {
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, Object> loginData) {
        try {
            ...
            if (isSuccess) {
                // 🔥 드디어 진짜 JWT 토큰 발급!
                String token = jwtService.generateToken(userId, isAdmin);
